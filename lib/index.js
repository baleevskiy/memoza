...
const wrapReal = (fn, args, cb) => {
  ...
  const updatePromiseResolveArg = (data) => { memozaData.promise = 'resolve'; memozaData.promiseArg = data; };
...
module.exports = config => (fn, prefix = '') => {
  ...
  if (fn.__memoza === undefined) {
    ...
    fn.__memoza = (...args) => { // eslint-disable-line no-param-reassign
      // If memoza is disabled then just skip caching completely
      if (!enabled) {
        return fn(...args);
      }
